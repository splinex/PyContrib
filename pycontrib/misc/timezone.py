'''
Tools for date format manipulations
'''
...
IANA_DATA = '''"Africa/Abidjan","GMT","GMT","","","+00:00:00","+00:00:00","","","","+00:00:00"
...
class TimezoneConverter(Singleton):

    def initialize(self):
        data = csv.reader(IANA_DATA.split('\n'))
        self.timezone_data = dict(map(lambda r:(r[0], r[1:]), data))

    '''
    Convert TZBaseData/IANA format to old POSIX. Returns None in case of unknown TZ. 
    '''
    def iana_posix(self, iana_timezone):
...
#         TZ=EST5EDT,M3.2.0/2,M11.1.0
        ...
        try:
            if r[5] in ('', '+00:00:00'):
                posix_timezone = '{std}{std_offset:+}'.format(
                    std=r[0],
                    std_offset=int(r[4].split(':')[0]) * -1
                )

            else:
                posix_timezone = '{std}{std_offset:+}{dst}{dst_offset:+},{start}/{start_time},{end}/{end_time}'.format(
                    std=r[0],
                    std_offset=int(r[4].split(':')[0]) * -1,
                    dst=r[2],
                    dst_offset=int(r[5].split(':')[0]),
                    start='M{2}.{0}.{1}'.format(*r[6].split(';')),
                    start_time='{0:02d}:{1:02d}'.format(*map(lambda x: int(x), r[7].split(':'))),
                    end='M{2}.{0}.{1}'.format(*r[8].split(';')),
                    end_time='{0:02d}:{1:02d}'.format(*map(lambda x: int(x), r[9].split(':')))
                )
        except:
            return None

        return posix_timezone
